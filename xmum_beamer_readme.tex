...
% \XMUMSetTitleShadowOn                                   % 标题背景阴影设置方式1：开启（默认）
% \XMUMSetTitleShadowOff                                  % 标题背景阴影设置方式2：关闭
...
\begin{frame}{接口说明（一）}
    \begin{table}
        ...
        \begin{tabular}{p{0.45\linewidth}p{0.45\linewidth}}
            ...
            \texttt{\textbackslash begin\{frame\}[nobg]\{title\}} & 关闭当前帧背景 \\
            \texttt{\textbackslash begin\{frame\}[fragile,nobg]\{title\}} & 关闭当前帧背景（针对包含原样环境的帧） \\
            \midrule
            ...
            \midrule
            \multicolumn{2}{l}{\textbf{阴影设置}} \\
            \texttt{\textbackslash XMUMSetTitleShadowOn} & 开启标题背景阴影（默认） \\
            \texttt{\textbackslash XMUMSetTitleShadowOff} & 关闭标题背景阴影 \\
            \bottomrule
        \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}{接口说明（二）}
    \begin{table}
        ...
        \begin{tabular}{p{0.45\linewidth}p{0.45\linewidth}}
            ...
            \texttt{\textbackslash XMUMTitleFrame} & 生成标题页 \\
            \texttt{\textbackslash XMUMSingleLineFrame[true/false]\{title\}\{text\}} & 生成单行正文页（可选参数用于切换背景样式） \\
            \bottomrule
        \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}[nobg]{接口说明（三）}
    \begin{figure}
    \centering
    \includegraphics[width=0.66\textwidth]{img/img-0.png}
...
\begin{frame}[nobg]{接口示例（一）}
    ...
    \begin{columns}[T]
        \begin{column}{0.36\linewidth}
            \begin{figure}
                \centering
                \includegraphics[width=\linewidth]{img/img-1.png}
                \caption{默认长版校徽}
            \end{figure}
        \end{column}
        \begin{column}{0.36\linewidth}
...
\begin{frame}{常用文稿结构}
    \begin{enumerate}
        ...
        \item 若当前页包含 \texttt{lstlisting} 或 \texttt{verbatim} 等原样代码环境，则需将参数写为：\texttt{\textbackslash begin\{frame\}[fragile]} 或 \texttt{\textbackslash begin\{frame\}[fragile,nobg]\{帧标题\}}
        \item 可使用 \texttt{\textbackslash XMUMSingleLineFrame[true/false]\{帧标题\}\{单行文本\}} 生成单行正文页，并通过可选参数切换背景样式。
    \end{enumerate}
...
\begin{frame}[fragile,nobg]{代码块示例（关闭背景示例）}
\begin{lstlisting}
#include <bits/stdc++.h>
using namespace std;
...
\begin{frame}{单行正文页彩色背景说明}
    \vspace{-2pt}
    \begin{itemize}
        \item \texttt{\textbackslash XMUMSingleLineFrame} 包含一个可选参数，用于控制单行正文页的背景样式。
        \item 若需要保留当前模板背景，可写为：\\ \texttt{\textbackslash XMUMSingleLineFrame[true]\{帧标题\}\{单行文本\}}。
        \item 若需要使用彩色背景，可写为：\\ \texttt{\textbackslash XMUMSingleLineFrame[false]\{帧标题\}\{单行文本\}}。
        \item 默认值为 \texttt{false}，可直接写为：\\ \texttt{\textbackslash XMUMSingleLineFrame\{帧标题\}\{单行文本\}}。
        \item 文本默认字体大小为 \texttt{\textbackslash LARGE}，可在正文中自行调整。
    \end{itemize}
    \vspace{-2pt}
    \begin{block}{示例}
        \texttt{\textbackslash XMUMSingleLineFrame[true]\{感谢\}\{感谢阅读和使用此模板\}}
        \texttt{\textbackslash XMUMSingleLineFrame[false]\{祝愿\}\{祝愿马校生活愉快\}}
        \texttt{\textbackslash XMUMSingleLineFrame\{项目地址\} \\ \{\{\textbackslash Large\textbackslash url\{https://github.com/zeyu10/xmum-beamer-template\}\}\}}
    \end{block}
\end{frame}

\XMUMSingleLineFrame[true]{感谢}{感谢阅读和使用此模板}

\XMUMSingleLineFrame[false]{祝愿}{祝愿马校生活愉快}

\XMUMSingleLineFrame{项目地址}{{\Large\url{https://github.com/zeyu10/xmum-beamer-template}}}
